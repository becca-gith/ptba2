...
public function listePtbas(array $filters = []): array
{
    ...
    // Récupération des PTBA formatés
    $ptbas = $query->orderByDesc('ptbas.created_at')->get()
        ->map(function ($ptba) {
            return [
                'id' => $ptba->id,
                'libelle' => $ptba->libelle,
                'annee' => $ptba->annee_libelle,
                'type' => \App\Types\TypePtba::label($ptba->type) ?? null,
                'montant_global' => $ptba->montant_global,
                'montant_decaissement' => 0, // À calculer si nécessaire
    ...
    // Récupération des années uniques (à partir des PTBA ou de la table années)
